                    ...
                    if let Some(iface_name) = line.split_whitespace().nth(1) {
                        for iface in &mut interfaces {
                            if iface.name == iface_name {
                                ...
                                iface.description = format!("Wireless interface (phy: {})", 
...
impl PlatformScanner for NmcliScanner {
    async fn scan(&self, interface: &str, config: &ScanConfig) -> Result<Vec<PlatformNetworkInfo>> {
        ...
        let output = TokioCommand::new("nmcli")
            .args(["-t", "-f", "SSID,BSSID,SIGNAL,SECURITY,FREQ,RATE", "dev", "wifi", "list", "ifname", interface])
            .output()
            .await
...
impl ScanEngine {
    ...
    pub async fn scan_once(&mut self, config: ScanConfig) -> Result<ScanResult> {
        ...
        let network_infos: Vec<NetworkInfo> = networks.into_iter()
        ...
        let summary = ScanSummary {
            ...
            by_band,
            by_security,
            strongest_signal: if signals.is_empty() { None } else { Some(strongest as i32) },
    ...
    pub async fn monitor(
        ...
        loop {
            ...
            let filtered_result = if !track_bssids.is_empty() {
                ...
                filtered.networks.retain(|n| track_bssids.iter().any(|t| &n.bssid == t));
    ...
    pub async fn analyze(
        ...
        for (channel, networks) in channel_data {
            if networks.is_empty() { continue; }
            
            let band = if channel <= 14 { "2.4GHz" } else if channel <= 165 { "5GHz" } else { "6GHz" };
        ...
        let interference = channels.iter().any(|c| c.networks > 3 || c.overlap_score > 50.0);
        
        let mut recommendations = Vec::new();
        if interference {
            recommendations.push("High interference detected - consider changing channel".to_string());
        }
        
        let best_24: Vec<_> = channels.iter().filter(|c| c.band == "2.4GHz" && c.networks <= 1).collect();
        let best_5: Vec<_> = channels.iter().filter(|c| c.band == "5GHz" && c.networks <= 1).collect();
        
        if !best_24.is_empty() {
            recommendations.push(format!("Best 2.4GHz channels: {}", 
                best_24.iter().map(|c| c.channel.to_string()).collect::<Vec<_>>().join(", ")));
        }
        if !best_5.is_empty() {
            recommendations.push(format!("Best 5GHz channels: {}", 
                best_5.iter().map(|c| c.channel.to_string()).collect::<Vec<_>>().join(", ")));
        }
...
impl OutputFormatter {
    ...
    pub async fn output_analysis(&self, result: &AnalysisResult, output_file: Option<&str>) -> Result<()> {
        let content = match self.format {
    ...
    pub fn format_text(&self, result: &ScanResult) -> String {
        ...
        out.push_str(&format!("  Total networks: {}\n", result.summary.total_networks));
        out.push_str(&format!("  Strongest signal: {} dBm\n", result.summary.strongest_signal.unwrap_or(0)));
        ...
        for net in &result.networks {
            let ssid_display = if net.ssid.is_empty() { "<hidden>".to_string() } else { net.ssid.clone() };
    ...
    pub fn format_csv(&self, result: &ScanResult) -> Result<String> {
        ...
        wtr.write_record(&[
            "bssid", "ssid", "signal_dbm", "frequency_mhz", "channel", 
            "band", "width", "security", "wps", "quality_percent", 
            "noise_floor_dbm", "snr_db", "vendor", "last_seen"
        ])?;
        ...
        for net in &result.networks {
            wtr.write_record(&[
                ...
                &net.noise_floor_dbm.map_or("".to_string(), |v| v.to_string()),
    ...
    pub fn format_analysis_text(&self, result: &AnalysisResult) -> String {
        ...
        out.push_str(&format!("Interference detected: {}\n", result.interference_detected));
        out.push_str("\n");
        ...
        for ch in &result.channels {
            out.push_str(&format!(
                ...
                ch.channel, ch.band, ch.networks, ch.avg_signal, 
                ch.max_signal, ch.min_signal, ch.overlap_score, ch.utilization_percent
            ));
        }
    ...
    pub fn format_analysis_csv(&self, result: &AnalysisResult) -> Result<String> {
        ...
        wtr.write_record(&[
            "channel", "band", "networks", "avg_signal", "max_signal", 
            "min_signal", "overlap_score", "utilization_percent"
        ])?;
...
#[command(about = "CLI WiFi scanner & signal analyzer - like iwlist + nmcli but with JSON/CSV output and continuous monitoring")]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Scan { interface, format, output, min_signal, band, security, sort, show_hidden } => {
        ...
        Commands::Monitor { interface, interval, format, output, count, track } => {
            let scan_config = ScanConfig {
                interface,
                min_signal: None,
                band_filter: None,
            ...
            engine.monitor(scan_config, Duration::from_millis(interval), count, track, format, output).await?;
        }
        ...
        Commands::Info { network, interface } => {
            ...
            let result = engine.scan_once(scan_config).await?;
            if let Some(net) = result.networks.iter().find(|n| n.bssid == network || n.ssid == network) {
        ...
        Commands::Analyze { interface, duration, format, output } => {
            let analysis = engine.analyze(interface, Duration::from_secs(duration)).await?;
            let formatter = OutputFormatter::new(format);
            formatter.output_analysis(&analysis, output.as_deref()).await?;
        }
